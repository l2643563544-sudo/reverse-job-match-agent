...
- 推荐榜默认输出 5 条，理由简洁。
- 实时采集单独运行约 4.75 秒。
- 最终版实时采集 + DeepSeek 完整链路补测约 61.09 秒，尚未稳定达到 30 秒目标。
- 4 项自动测试通过。
...
## 2026-09-14 UI/UX 前端重构

### 背景

用户提供 Light / Dark 两张视觉参考图，要求把当前招聘后台风格升级为专业 AI 人岗匹配工作台。

### 目标

保留现有业务数据、API 和交互，仅重构前端布局、主题系统、信息层级和展示状态。

### 改动文件

- 重写 `static/index.html`
- 重写 `static/styles.css`
- 重写 `static/app.js`
